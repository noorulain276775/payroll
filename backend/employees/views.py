...
from .models import EmployeeDetails, PayrollRecord
from .serializers import EmployeeDetailsSerializer, ColleagueSerializer, PayrollRecordSerializer
...
def view_employee(request):
    ...
    try:
        employee_details = EmployeeDetails.objects.get(user=request.user)
...
# Admin can update employee details (except designation and department)
@api_view(['PUT', 'PATCH'])
@permission_classes([IsAuthenticated])
...
def view_colleagues(request):
    if request.user.user_type == 'Employee':
        colleagues = EmployeeDetails.objects.exclude(user=request.user)  # Exclude the logged-in user
...
# Admin-only view to update employee details (including salary and payroll)
@api_view(['PUT', 'PATCH'])
@permission_classes([IsAuthenticated])
def update_employee(request):
    if request.user.user_type != 'Admin' and request.user.user_type != 'Employee':
        return Response({"detail": "Only admins or employees can update details."}, status=status.HTTP_403_FORBIDDEN)
    try:
        employee_details = EmployeeDetails.objects.get(user=request.user)
...
# Function to send the salary slip to an employee via email
